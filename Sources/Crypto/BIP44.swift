...
public class HDNode {
    ...
    public init?(_ data: Data) {
        ...
        if serializePrivate {
            ...
            let pubKey = Secp256k1.createPublicKey(from: privateKey!.makeBytes())
    ...
    public init?(seed: Data) {
        ...
        guard Secp256k1.verify(seckey: privKeyCandidate.makeBytes()) else {
            return nil
        }
        let pubKeyCandidate = Secp256k1.createPublicKey(from: privKeyCandidate.makeBytes())
        publicKey = Data(pubKeyCandidate)
    ...
    public func derive (index: UInt32, derivePrivateKey:Bool, hardened: Bool = false) -> HDNode? {
        if derivePrivateKey {
            if self.hasPrivate { // derive private key when is itself extended private key
                ...
                guard Secp256k1.verify(seckey: privKeyCandidate.makeBytes()) else {return nil }
                let pubKeyCandidate = Secp256k1.createPublicKey(from: privKeyCandidate.makeBytes(), compressed: true)
                ...
                newNode.publicKey = Data(pubKeyCandidate)
                newNode.privateKey = privKeyCandidate
                newNode.childNumber = trueIndex
                guard let fprint = try? SHA3(variant: .keccak256).calculate(for: self.publicKey.sha256().makeBytes())[0..<4] else {
                    return nil
                }
                
                newNode.parentFingerprint = Data(fprint)
                var newPath = String()
            ...
            guard Secp256k1.verify(seckey: tempKey.makeBytes()) else {return nil }
            let pubKeyCandidate = Secp256k1.createPublicKey(from: tempKey.makeBytes(), compressed: true)
            guard pubKeyCandidate[0] == 0x02 || pubKeyCandidate[0] == 0x03 else {return nil}
            // TODO
            guard self.depth < UInt8.max else {return nil}
            let newNode = HDNode()
...
public class BIP44 {
    static public func  createPrivateKeyBySeed(path: String, seed: Data) -> Data? {
        guard let master = HDNode(seed: seed) else {return nil}
        let sub = master.derive(path: path)!
